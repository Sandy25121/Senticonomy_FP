...
if page == "Data Preprocess and Analysis":
    ...
    def preprocess_text(text):
        ...
        # Debugging: Check if text is empty after cleaning
    ...
    @st.cache_data
    def load_data():
        df = pd.read_csv("Cleaned_News_DataSet.csv", encoding='utf-8')
        ...
        # Additional safeguard: Filter out short or empty strings post-cleaning
        df['short_description_clean'] = df['short_description_clean'].apply(lambda x: x if len(x.split()) > 1 else '')

        df = df[df['short_description_clean'].str.strip() != '']
    ...
    if df.empty:
        st.error("The cleaned dataset is empty after preprocessing. Please check the input data or preprocessing steps.")
        st.stop()

    # Compute clusters using KMeans and TfidfVectorizer
    @st.cache_resource
